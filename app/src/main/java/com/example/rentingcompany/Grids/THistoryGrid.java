...
public class THistoryGrid extends BaseAdapter {
    private final ArrayList<Property> propertiesArrayList = new ArrayList<Property>();
    private Context mContext;

    public THistoryGrid(Context c, ArrayList<Property> propertiesArrayList) {
        mContext = c;
...
//        if (convertView == null) {
        ...
        //surface_area_textView.setText("Renting Period: ");
        DataBaseHelper DB = new DataBaseHelper(mContext, "EXP4", null, 1);
        Cursor cursor = DB.getReadableDatabase().rawQuery("Select AGENCYNAME, STARTDATE from RENTINGAGENCY, HAVE, CONTRACT, TENANT WHERE HAVE.EMAIL = RENTINGAGENCY.EMAIL AND CONTRACT.EMAIL = TENANT.EMAIL AND HAVE.POSTALADDRESS LIKE '" + X.getPostalAddress() + "'", null);

        SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/yyyy");
        if (cursor.moveToNext()) {
            Date firstDate = null;
            try {
                firstDate = sdf.parse(cursor.getString(1));
            ...
            info_textView.setText("Renting Agency Name: " + cursor.getString(0) + "\nRenting Period: " + (diff / 1000 / 60 / 60 / 24) + " Day/s");
        } else
